...
const (
	...
	FrozenDashBoardCacheKey   = "frozen_dashboard"
...
var frozenAllDashboardOnce sync.Once
var frozenAllDashboard *struct {
	leaderboard []xsuportal.LeaderBoardTeam
	jobResults  []xsuportal.JobResult
}
...
func main() {
	...
	var err error
	contestStatus, err = getCurrentContestStatusFromDB(nil, db)
	if err != nil {
		log.Fatalf("get contest: %v", err)
	}
...
func getTeams(teamIDs []int64) (map[int64]xsuportal.Team, error) {
	...
	err = db.Select(&teams, sql, params...)
	if err != nil {
		return nil, err
	}

	teamMap := make(map[int64]xsuportal.Team, len(teams))
	for i := range teams {
...
func (*AdminService) ListClarifications(e echo.Context) error {
	...
	if ok, err := loginRequiredByContestant(e, contestant, &loginRequiredOption{}); !ok {
		return wrapError("check session", err)
	}
	...
	if len(clarifications) > 0 {
		teamIDs := make([]int64, len(clarifications))
		for i := range clarifications {
			teamIDs[i] = clarifications[i].TeamID
		}
		teamMap, err = getTeams(teamIDs)
		if err != nil {
	...
	for _, clarification := range clarifications {
		team := teamMap[clarification.TeamID]
...
func (*RegistrationService) DeleteRegistration(e echo.Context) error {
	...
	if ok, err := loginRequiredByContestant(e, contestant, &loginRequiredOption{Team: true, Lock: true}); !ok {
		return wrapError("check session", err)
	}
...
func getCurrentContestStatus(e echo.Context, db sqlx.Queryer) (*xsuportal.ContestStatus, error) {
	...
	if now.Before(cs.RegistrationOpenAt) {
		cs.StatusStr = "standby"
		cs.Status = resourcespb.Contest_STANDBY
	} else if now.Before(cs.ContestStartsAt) {
		cs.StatusStr = "registration"
		cs.Status = resourcespb.Contest_REGISTRATION
	} else if now.Before(cs.ContestEndsAt) {
		cs.StatusStr = "started"
		cs.Status = resourcespb.Contest_STARTED
	} else {
		cs.StatusStr = "finished"
...
func loginRequiredByContestant(e echo.Context, contestant *xsuportal.Contestant, option *loginRequiredOption) (bool, error) {
	if contestant == nil {
		return false, halt(e, http.StatusUnauthorized, "ログインが必要です", nil)
	}
	if option.Team {
...
func makeLeaderboardPB(e echo.Context, teamID int64) ([]byte, error) {
	...
	if contestFrozen {
		if isSame {
			if res, ok := cacheStore.Get(FrozenDashBoardCacheKey); ok {
				return res.([]byte), nil
			}
		}

		frozenAllDashboardOnce.Do(func() {
			tx, err := db.Beginx()
			if err != nil {
			...
			err = tx.Select(&leaderboard, query, contestFreezesAt, contestFreezesAt)
			if err != sql.ErrNoRows && err != nil {
				fmt.Printf("select leaderboard: %v\n", err)
				return
			}
			...
			jobResultsQuery := "SELECT\n" +
				...
				"ORDER BY\n" +
				"  `finished_at`"
			err = tx.Select(&jobResults, jobResultsQuery, contestFreezesAt)
			if err != sql.ErrNoRows && err != nil {
				fmt.Printf("select job results: %v\n", err)
				return
			}
		...
		if isSame {
			...
			for _, jobResult := range frozenAllDashboard.jobResults {
			...
			for _, team := range frozenAllDashboard.leaderboard {
			...
			cacheStore.Set(FrozenDashBoardCacheKey, res, cache.NoExpiration)
			return res, nil
		}

		v, err, _ := dashboardGroup.Do("frozen"+name, func() (interface{}, error) {
			dashboard := *frozenAllDashboard
			...
			var leaderboardTeam xsuportal.LeaderBoardTeam
			var jobResults []xsuportal.JobResult
			query := "SELECT\n" +
				"  `teams`.`id` AS `id`,\n" +
				"  `teams`.`name` AS `name`,\n" +
			...
			// 差分書き換え
			for i := range dashboard.leaderboard {
				if dashboard.leaderboard[i].ID == leaderboardTeam.ID {
					dashboard.leaderboard[i] = leaderboardTeam
					break
				}
			}
			sort.Slice(dashboard.leaderboard, func(i, j int) bool {
				if dashboard.leaderboard[i].LatestScore.Int64 == dashboard.leaderboard[j].LatestScore.Int64 {
					return dashboard.leaderboard[i].LatestScoreMarkedAt.Time.Before(dashboard.leaderboard[j].LatestScoreMarkedAt.Time)
				}
				return dashboard.leaderboard[i].LatestScore.Int64 > dashboard.leaderboard[j].LatestScore.Int64
			})

			newJobResults := make([]xsuportal.JobResult, 0, len(dashboard.jobResults)+len(jobResults))
			newJobResults = append(newJobResults, jobResults...)
			for i := range dashboard.jobResults {
				if dashboard.jobResults[i].TeamID != teamID {
					newJobResults = append(newJobResults, dashboard.jobResults[i])
				}
			}
			sort.Slice(newJobResults, func(i, j int) bool {
				return newJobResults[i].FinishedAt.Before(newJobResults[j].FinishedAt)
			})
			dashboard.jobResults = newJobResults
			// ---

			teamGraphScores := make(map[int64][]*resourcespb.Leaderboard_LeaderboardItem_LeaderboardScore)
			for _, jobResult := range dashboard.jobResults {
				teamGraphScores[jobResult.TeamID] = append(teamGraphScores[jobResult.TeamID], &resourcespb.Leaderboard_LeaderboardItem_LeaderboardScore{
					Score:     jobResult.Score,
					StartedAt: timestamppb.New(jobResult.StartedAt),
					MarkedAt:  timestamppb.New(jobResult.FinishedAt),
			...
			pb := &resourcespb.Leaderboard{}
			for _, team := range dashboard.leaderboard {
				t, _ := makeTeamPB(db, team.Team(), false, false)
				item := &resourcespb.Leaderboard_LeaderboardItem{
					Scores: teamGraphScores[team.ID],
					BestScore: &resourcespb.Leaderboard_LeaderboardItem_LeaderboardScore{
	...
	v, err, _ := dashboardGroup.Do("all"+name, func() (interface{}, error) {
		tx, err := db.Beginx()
		if err != nil {
			return nil, fmt.Errorf("begin tx: %w", err)
		}
